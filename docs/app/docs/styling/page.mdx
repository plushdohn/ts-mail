...
`ts-email` provides a flexible way to style your email HTML output. You can customize colors, fonts, borders, and other visual aspects by passing a `styles` object when building your email, or override specific style tokens as needed.
...
```ts
import { buildEmail } from "ts-email";
